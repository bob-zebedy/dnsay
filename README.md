...
| 参数 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| `--name` | string | （自动生成） | 昵称；显式指定时重名直接退出 |
...
| `n` | 查询组内昵称（预留） | 无 | base32(`name1\x00name2\x00...`) |
| `l` | 离开 | 无 | `ok` |
...
| TXT 单条字符串上限 | 200 字符 (base32) | `server/main.go: maxTXTLength` |
...
└─────────────────────────────────────────────────────────┘
         │                      │
         ▼                      ▼
  nonce(12) || ct(N+16)   nonce(12) || ct(N+16)
```
...
## 共享常量参考（`shared/crypto.go`）

```go
const (
    NonceSize     = 12   // AES-GCM nonce 长度
    SIDSize       = 8    // 会话 ID 长度
    LabelSegLen   = 30   // DNS 标签内单段载荷长度
    GCMTagSize    = 16   // GCM 认证 tag 长度
    MinE2EBlobLen = 28   // E2E blob 最小长度 (nonce + tag)
